...
namespace LogisticLab
{
	public class LogisticClassifier
	{
		...
		public double[] TrainWeights(double[][] trainData, int maxEpochs, int seed, double alpha1, double alpha2)
		{
			...
			int epoch = 0;
			double minX = -20.0; // for each weight. assumes data has been normalized about 0 ORGIN -10
			double maxX = 20.0;//10
			double w = 0.729; // inertia weight 0.729
	...
	class MainClass
	{
		public static void Main(string[] args)
		{
			int numFeatures = 6;
			int numRows = 2050;
			int seed = 28;  // interesting seeds: 28, 32, (42), 56, 58, 63, 91
			...
			string[] lines = System.IO.File.ReadAllLines(@"/Users/zelongzhao/Desktop/test5.csv");

			// Display the file contents by using a foreach loop.
			double[][] allData = new double[14710][];
			int i = 0;
			foreach (string line in lines)
			{
				...
				for (int j = 0; j < nums.Length - 2; j++)
				{
					...
					//Console.Write(allData[i][j] + " ");

				}
				allData[i][6] = double.Parse(nums[6]);
				//Console.Write(allData[i][6] + " ");
				//Console.WriteLine();
				i++;
